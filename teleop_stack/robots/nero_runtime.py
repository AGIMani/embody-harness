...
from teleop_palm_plane.palm_plane import xyzw_to_wxyz
...
class NeroTeleopMappingConfig:
    translation_scale_xyz: tuple[float, float, float] = (0.15, 0.15, 0.15)
    workspace_origin_xyz: tuple[float, float, float] = (0.0, 0.0, 0.0)
    input_axis_map: AxisMap = ("z", "x", "y")
    use_teleop_orientation: bool = True
    fixed_quaternion_wxyz: tuple[float, float, float, float] | None = None

    def map_position_absolute(self, pose: Pose7) -> tuple[float, float, float]:
        mapped = _map_vec3_axes(pose.position_xyz, self.input_axis_map)
        return tuple(
