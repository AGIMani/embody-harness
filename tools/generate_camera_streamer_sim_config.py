...
def _default_output_path(isaac_teleop_root: Path) -> Path:
    return isaac_teleop_root / "examples" / "camera_streamer" / "build" / "add_scene_sim_screen_xr.yaml"


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate a minimal IsaacTeleop camera_streamer XR config for the add_scene_glb sim screen.",
    )
    ...
    payload = {
        ...
        "display": {
            ...
            "monitor": {
                "width": int(args.width),
                "height": int(args.height),
                "title": "add_scene_glb sim screen",
                "padding": 4,
                "stream_timeout": 2.0,
            },
            "xr": {
