...
COMBINED_ROOT_TO_BASE_STL_XYZ = (0.25, -0.25, 0.0)
...
def _add_empty_link(root: ET.Element, *, name: str) -> None:
...
def _set_prefixed_visual_color(
    root: ET.Element,
    *,
    link_prefix: str,
    color_rgba: tuple[float, float, float, float],
    ...
    for link in root.findall("link"):
        ...
        for visual in link.findall("visual"):
            ...
            material.attrib["name"] = f"{link_prefix.rstrip('_')}_silver_white"
            color = material.find("color")
            if color is None:
                color = ET.SubElement(material, "color")
            color.attrib["rgba"] = _fmt(color_rgba)
...
def build_combined_urdf(output: Path, *, hand_side: str) -> Path:
    ...
    _add_empty_link(root, name="combined_root")
    _add_mesh_link(
        root,
        name="base_stl",
        mesh=base_mesh,
    ...
    _add_fixed_joint(
        root,
        name="combined_root_to_base_stl",
        parent="combined_root",
        child="base_stl",
        xyz=COMBINED_ROOT_TO_BASE_STL_XYZ,
        rpy=(0.0, 0.0, 0.0),
    )
    ...
    _set_prefixed_visual_color(
        root,
        link_prefix=hand_prefix,
        color_rgba=(0.86, 0.88, 0.88, 1.0),
    )
