...
if [[ "${MODE}" == "image" ]]; then
    CONTAINER_NAME="${CLOUDXR_WEB_CONTAINER_NAME:-cloudxr-web-app-harness}"
...
CONFIG_FILE="${WEBXR_DIR}/webpack.harness.local.js"
cat > "${CONFIG_FILE}" <<EOF
const fs = require('fs');
...
PREVIOUS_HASH_FILE="${WEBXR_DIR}/node_modules/.harness-deps-hash"
