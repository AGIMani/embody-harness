...
usage() {
    cat <<'EOF'
Usage: scripts/run_add_scene_vr_output.sh [options]

Streams the Genesis viewer into Quest/CloudXR as an XR plane and enables the
IsaacTeleop hand skeleton overlay.

Options:
  --device /dev/videoX        V4L2 loopback output device (default: /dev/video44)
  --display :N[.S]            X11 display containing the Genesis viewer
...
if [[ -n "$CAPTURE_DISPLAY" ]]; then
    ...
    status_error "DISPLAY is empty. Pass --display :N after confirming the Genesis viewer display with echo \$DISPLAY."
fi

if [[ -e "$DEVICE" ]]; then
    status_ok "sim screen device=$DEVICE"
else
    video_nr="${DEVICE#/dev/video}"
